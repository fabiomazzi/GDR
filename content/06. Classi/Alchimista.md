---
Tipo: classe
PV: "6"
Abilità: Conoscenze, Magia
livello1: |-
  - Reazione: sei entro 10m dall’attaccante. Usa un composto comodo sull’attaccante.
  - Sai usare un distillatore modificato magicamente: occupa uno slot comodo, contiene il **necessario per creare [[11.1 Composti|Composti]] e [[11.3 Veleni|Veleni]]; ne contiene un numero pari alla tua Conoscenze (sono considerati comodi)**. Quando crei un composto puoi invece crearne tanti quanti ne tiene il tuo distillatore facendoli riposare solo 1 ora, ma devono essere composti che non hai già creato durante questo riposo.
  - Puoi usare Conoscenze invece che l’abilità richiesta per creare Composti e i Veleni.
livello2: "-Azione: un numero di volte per riposo pari al tuo livello, crea un composto o un veleno, usalo immediatamente come parte di quest’Azione o diventa inerte."
livello3: "- Hai accesso ai [Composti da Alchimista](app://obsidian.md/11.%20Composti%20e%20Veleni#Composti%20da%20alchimista)."
livello4: "- Un numero di volte per riposo pari al tuo livello da Alchimista, quando usi un composto con un’Azione puoi invece usarne due."
---
Descrizione della classe, tira le bombette, fa i veleni, tipo alchimista pazzo, non so devo scrivere un po' di descrizione per vedere come sta.

**PV:** 6  
**Abilità:** Conoscenze, Magia

---
### Livello 1
- **Reazione**: sei entro 10m dall’attaccante. Usa un composto comodo sull’attaccante.
- Sai usare un distillatore modificato magicamente: occupa uno slot comodo, contiene il necessario per creare [Composti](app://obsidian.md/12.%20Composti%20e%20Veleni#Composti) e [Veleni](app://obsidian.md/12.%20Composti%20e%20Veleni#Veleni); ne contiene un numero pari alla tua Conoscenze (sono considerati comodi). Quando crei un composto puoi invece crearne tanti quanti ne tiene il tuo distillatore facendoli riposare solo 1 ora, ma devono essere composti che non hai già creato durante questo riposo.
- Puoi usare Conoscenze invece che l’abilità richiesta per creare Composti e i Veleni.
### Livello 2
- Azione: un numero di volte per riposo pari al tuo livello, crea un composto o un veleno, usalo immediatamente come parte di quest’Azione o diventa inerte.
### Livello 3
- Hai accesso ai [Composti da Alchimista](app://obsidian.md/12.%20Composti%20e%20Veleni#Composti%20da%20alchimista).
### Livello 4
- Un numero di volte per riposo pari al tuo livello da Alchimista, quando usi un composto con un’Azione puoi invece usarne due.
